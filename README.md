...
Viewing is public. Every page opening or refresh starts in view-only mode, even with a saved login. Editor access requires entering the existing designated owner's Supabase password for that page. Row-level security restricts progress changes to that owner. The browser contains only the public publishable key.
